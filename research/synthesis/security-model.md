...
### 案例 5：OpenCLI 的全权委托模型——安全反面教材（2026.03）

> 来源：GitHub jackwener/opencli 仓库源码调研（2026-03-21）

**背景**：OpenCLI 通过 Chrome 扩展 + 本地 Daemon + WebSocket 将网站和 Electron 应用 CLI 化，复用浏览器已登录的 session 让 Agent 操控 Web 应用。项目一周内获 3,300+ stars。

**安全缺陷清单**：

| 问题 | 风险等级 | 说明 |
|------|---------|------|
| **Daemon 无身份验证** | 高 | 监听 localhost:19825，无任何认证——本机任何进程都可发送 WebSocket 命令，冒充用户操作浏览器 |
| **无沙箱** | 高 | `evaluate()` 中的 JS 代码可在页面中做任何事，无执行隔离 |
| **无权限确认** | 中 | 运行 CLI 命令即视为授权，无细粒度权限控制，Agent 拥有用户在浏览器中的全部权限 |
| **XHR 全局拦截** | 中 | 通过猴子补丁全局 `fetch` 和 `XMLHttpRequest`，理论上可捕获页面所有网络请求 |
| **CORS 设为 \*** | 低 | 虽为本地服务，但不够严谨 |

**全权委托模型的本质问题**：

OpenCLI 的安全模型是"Account-safe"——凭证不离开浏览器。但这掩盖了更深层的问题：**Agent 默认获得用户在浏览器中的全部权限**，且没有任何分级机制。用户能在 B 站发弹幕，Agent 就能；用户能在社交平台发消息，Agent 就能。这是典型的 ASI03（困惑代理人）风险放大。

**与 MCP 安全事件的共性**：

- **无认证本地服务**：与"492 个零认证暴露的 MCP Server"如出一辙
- **信任描述而非验证行为**：用户信任 CLI 命令描述，但无法验证实际执行的 JS 代码
- **生态快速增长超过安全审查**：一周 3,300+ stars，267/300+ commits 集中在一人，安全设计滞后

**正面实践——Automation Window Isolation**：

OpenCLI 的 Chrome 扩展实现了一个值得借鉴的隔离机制：所有操作在专用的 Automation Window 中执行，不干扰用户正常浏览，窗口 30 秒空闲后自动关闭。这是轻量级操作隔离的有效实践，但仅解决了"不干扰用户"问题，未解决"权限过大"的根本问题。

**教训**：
- **"凭证不离开浏览器"不等于安全**——关键在于 Agent 能用这些凭证做什么
- **网络操作不可撤回**：与本地文件操作（可 undo）不同，通过浏览器执行的操作（发消息、提交表单、社交平台操作）发出即不可撤回
- **全权委托是最危险的默认值**——Agent 工具应默认最小权限，而非继承用户全部权限
...
| L4+ 网络不可撤回操作 | 浏览器代理操作（发消息、提交表单、社交平台操作） | 强制确认 + 预览 + 操作日志 | OpenCLI 等浏览器代理场景。与本地 L4 不同：本地文件删除可从备份恢复，网络操作（发送的消息、提交的表单）发出即不可撤回 |
