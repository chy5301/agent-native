...
**核心设计**：
- 用户侧 Skill **只做触发和通信**，明确禁止扩写 prompt 或拆解任务
- 后端 Agent 完成全部创作工作（分镜、选模型、调参数）
- 两个入口**共享同一 project/session 状态**——Agent 创建的内容在画布上可见

**与其他方案的关键区别**：

| 维度 | LibTV | A2UI/AG-UI | Streamlit/Gradio | CLI + Web Dashboard |
|------|-------|-----------|-----------------|-------------------|
| 层次 | 产品级实现 | 协议级规范 | 框架级工具 | 架构模式 |
| 人类界面 | 原生 Web 画布 | Agent 动态生成 | 框架生成 | 独立构建 |
| Agent 界面 | OpenClaw Skill | 协议层 | Python API | CLI |
| 后端共享 | ✅ 完全共享 | ✅ 协议级 | ✅ Python 层 | ❌ 仅共享 API |
| 业务逻辑位置 | 后端 Agent | 视实现而定 | 应用代码 | 视实现而定 |
| IP 保护 | ✅ 核心能力闭源 | N/A | ❌ 代码可见 | 视实现而定 |

**新模式的意义**："接口而非大脑"——对外发布调用接口，核心能力留在后端。这解决了 Skill 生态中开放性与商业壁垒的矛盾。
...
LibTV 是首批将 Agent-Native 双模设计投入生产的案例。其中**部分模式可迁移**，部分则是画布编排场景特有的：

**可迁移的模式**：
1. **"Skill 即薄中继"**——用户侧 Skill 只做触发和通信，业务逻辑集中后端。对任何需要保护核心能力的 SaaS 转 Skill 产品都适用
2. **IM 异步会话**——对耗时较长、多轮交互的任务普遍适用，不限于创意生成
3. **"接口而非大脑"**——开源 Skill 建生态，核心能力留后端。解决了 Skill 生态中开放性与商业壁垒的矛盾

**场景绑定，不可直接迁移**：
1. **无限画布作为人类入口**——画布编排是视频/设计工作流特有模式，其他类型工具需要找到自己的人类入口形态（Dashboard、TUI、编辑器插件等）
2. **"Agent 出初稿，人类在画布上精调"**——依赖画布的可视化编辑能力，非所有场景都有对应的精调界面
3. **后端 Agent 全权决策**——对确定性要求高的工具（金融、运维），用户可能需要更多控制权

**一般性结论**：Agent 入口趋同（CLI/Skill/MCP），人类入口因场景而异。"两个入口，一个内核"的理念成立，但具体的人类入口设计需要因地制宜。
